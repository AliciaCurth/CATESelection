...
class OracleTEScorer(_BaseTEScorer):
    ...
    def _score(self, estimator, X, y_factual, w_factual, p_true=None, t_true=None,
        ...
        if sample_weight is not None:
            return self._sign * self._score_func(t_true, t_pred, sample_weight=sample_weight,
                                               )
        else:
...
class OraclePOScorer(_BaseTEScorer):
    ...
    def _score(self, estimator, X, y_factual, w_factual, p_true=None, t_true=None,
        ...
        if sample_weight is not None:
            ...
            return self._sign * (self._score_func(t_true[0], mu0, sample_weight=sample_weight,
                                               ) + self._score_func(t_true[1], mu1,
                                                                    sample_weight=sample_weight,
                                               )
                                 )
        else:
            return self._sign * (self._score_func(t_true[0], mu0) + self._score_func(t_true[1],
...
class OraclePseudoOutcomeScorer(_BaseTEScorer):
    ...
    def _score(self, estimator, X, y_factual, w_factual, p_true=None, t_true=None,
               sample_weight=None, t_score=None, mu0_true=None, mu1_true=None):
        ...
        t_pseudo = pseudo_outcome_transformation(y_factual, w_factual, p_true, mu0_true, mu1_true,
