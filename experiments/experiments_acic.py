...
def download_orig_acic():
    ...
    download_if_needed(data_path / "data_cf_all.tar.gz", file_id=ACIC_ORIG_ID, unarchive=True,
                       unarchive_folder=data_path
                       )
...
def get_acic_covariates(pre_trans: bool = False,
...
def get_acic_data_orig(simu_num, seed, n_test, subset_train=None, pre_trans=True,
                       subset_cols='all'):
    ...
    # split data
    X, X_t, y, y_t, w, w_t, mu0, mu0_t, mu1, mu1_t, cate_in, cate_out = split_data(X, y_full,
                                                                                   w_full, mu0_full,
                                                                                   mu1_full,
                                                                                   cate_full,
                                                                                   n_test=n_test,
...
def split_data(X_full, y_full, w_full, mu0_full, mu1_full, cate_full, n_test=0.8,
               random_state=42,
    ...
    if subset_train is not None:
        X, y, w, mu0, mu1, cate_in = X[:subset_train, :], y[:subset_train], w[:subset_train], \
                                     mu0[:subset_train], mu1[:subset_train], cate_in[:subset_train]

    return X, X_t, y, y_t, w, w_t, mu0, mu0_t, mu1, mu1_t, cate_in, cate_out


def acic_simu(i_exp, n_train=1000, n_test=500, error_sd: float = 1,
              sp_lin: float = 0.6, sp_nonlin: float = 0.3,
              sp_1: float = 0, p_0: float = 0, ate_goal: float = 0, inter: int = 3,
              inter_t: int = 0,
              return_ytest: bool = False,
              col_type='all', xi=0.5, propensity_type='random',
              rel_prop=0.5,
              pre_trans=True, nonlin_scale=0.1, r_spec=False, prop_truespec=True):
    ...
    # create dgp
    coeffs_ = [0, 1]
    beta_0 = np.random.choice(coeffs_, size=n_cov, replace=True, p=[1 - sp_lin * cov_ratio,
                                                                    sp_lin * cov_ratio])
    intercept = np.random.choice([x for x in np.arange(-1, 1.25, 0.25)])
    beta_1 = np.random.choice(coeffs_, size=n_cov, replace=True,
                              p=[1 - sp_1 * cov_ratio, sp_1 * cov_ratio])
    ...
    if sp_nonlin > 0:
        if ((not col_type == 'other') and (not col_type == 'binarized')) and (
                not col_type == 'missp_out'):
            beta_sq = np.random.choice(coefs_sq, size=N_NUM_COLS, replace=True,
                                       p=[1 - sp_nonlin * cov_ratio,
                                          sp_nonlin * cov_ratio])
        ...
        if inter > 0:
            ...
            if inter > 2:
                ...
                for i in range(0, n_cov - 4, 4):
                    inter_list.append(
                        X[:, ind_c[i]] * X[:, ind_c[i + 1]] * X[:, ind_c[i + 2]] * X[:,
                                                                                   ind_c[i + 3]])

            X_inter = np.array(inter_list).T
            n_inter = X_inter.shape[1]
            beta_inter = np.random.choice(coefs_sq, size=n_inter, replace=True,
                                          p=[1 - sp_nonlin * cov_ratio, sp_nonlin * cov_ratio])
    ...
    if inter_t > 0:
        ...
        if inter_t > 2:
            ...
            for i in range(0, n_cov - 4, 4):
                inter_list_t.append(
                    X[:, ind_c[i]] * X[:, ind_c[i + 1]] * X[:, ind_c[i + 2]] * X[:, ind_c[i + 3]])

        X_inter_t = np.array(inter_list_t).T
        n_inter = X_inter_t.shape[1]
        beta_inter_t = np.random.choice(coefs_sq, size=n_inter, replace=True,
                                        p=[1 - sp_1 * cov_ratio, sp_1 * cov_ratio])
    ...
    if propensity_type == 'random':
        ...
        exponent = zscore(np.dot(X_prop, coeff)) if np.sum(coeff) > 0 else np.zeros_like(np.dot(
            X_prop, coeff))
        p = expit(xi * exponent)
    ...
    X_train, y_train, w_train, cate_train, mu0_train, mu1_train, p_train = X_return[ind[:(n_train)],
                                                                           :], \
                                                                           y[ind[:(n_train)]], \
                                                                           w[ind[:(n_train)]], cate[
                                                                               ind[:(n_train)]], \
    ...
    X_test, mu_0_t, mu_1_t, cate_t, p_t = X_return[ind_test, :], mu_0[ind_test], mu_1[ind_test], \
                                          cate[ind_test], p[ind_test]

    if return_ytest:
        y_test, w_test = y[ind_test], w[ind_test]
        return X_train, y_train, w_train, mu0_train, mu1_train, cate_train, p_train, X_test, \
               y_test, w_test, mu_0_t, mu_1_t, cate_t, p_t

    return X_train, y_train, w_train, mu0_train, mu1_train, cate_train, p_train, \
...
class AcicOrigGenerator(DataGenerator):
    def __init__(self, n_train=1000, n_val=500, n_test=500, subset_cols='all', simu_num=1):
        ...
        self.name = 'acicorig-' + str(simu_num) + '-' + str(n_train) + '-' + str(n_val) + '-' \
                    + str(n_test) + '-' + subset_cols

    def __call__(self, seed=42):
        X, X_t, y, y_t, w, w_t, mu0, mu0_t, mu1, mu1_t, cate_in, cate_out = get_acic_data_orig(
            simu_num=self.simu_num, n_test=self.n_test, subset_train=self.n_train + self.n_val,
            subset_cols=self.subset_cols, seed=seed)
        X_train, X_val, w_train, w_val, y_train, y_val, mu0_train, mu0_val, mu1_train, mu1_val, \
        cate_train, cate_val = train_test_split(X,
                                                w,
                                                y,
                                                mu0,
                                                mu1,
                                                cate_in,
                                                train_size=self.n_train,  # TODO
                                                random_state=seed)  # stratify on w
        ...
        data_val = X_val, y_val.squeeze(), w_val.squeeze(), mu0_val.squeeze(), \
                   mu1_val.squeeze(), cate_val.squeeze(), \
                   None
        data_test = X_t, y_t.squeeze(), w_t.squeeze(), mu0_t.squeeze(), \
                    mu1_t.squeeze(), cate_out.squeeze(), None

        return data_train, data_val, data_test


class AcicLinearGenerator(DataGenerator):
    def __init__(self, n_train=1000, n_val=1000, n_test=500,
                 sp_lin=0.6, sp_nonlin=0.3, sp_1=0, inter=True, xi=0.5, p_0=0,
                 error_sd=1, ate_goal=0, subset_cols='all', propensity_type='random',
                 trans_cov=True, inter_t=0, nonlin_scale=0.1,
                 rel_prop=0.5, r_spec=False, prop_truespec=True
                 ):
        self.n_train = n_train
        self.n_val = n_val
        self.n_test = n_test
        ...
        self.name = 'aciclinnobias' + ('rspec' if r_spec else '') + ('propfalsespec' if not
        prop_truespec else '') + '-' \
                    + str(n_train) + '-' + str(n_val) + '-' + str(n_test) + \
                    '-' \
                    + \
                    str(sp_lin) + '-' + str(sp_nonlin) + '-' + str(sp_1) + '-' + str(
            p_0) + '-' + str(
            inter) + '-' + str(inter_t) + '-' + str(nonlin_scale) + '-' + propensity_type \
                    + '-' + str(xi) + '-' + str(rel_prop) + '-' + subset_cols + '-' + str(
            trans_cov) + '-' + str(error_sd)

    def __call__(self, seed=42):
        X, y, w, mu0, mu1, cate, p, X_test, y_test, w_test, mu_0_test, mu_1_test, cate_test, \
        p_test = acic_simu(
            seed,
            ...
            r_spec=self.r_spec)
        X_train, X_val, w_train, w_val, y_train, y_val, mu0_train, mu0_val, mu1_train, mu1_val, \
        cate_train, cate_val, p_train, p_val = train_test_split(X,
                                                                w,
                                                                y,
                                                                mu0,
                                                                mu1,
        ...
        data_train = X_train, y_train.squeeze(), w_train.squeeze(), mu0_train.squeeze(), \
                     mu1_train.squeeze(), cate_train.squeeze(), p_train.squeeze()
        data_val = X_val, y_val.squeeze(), w_val.squeeze(), mu0_val.squeeze(), \
                   mu1_val.squeeze(), cate_val.squeeze(), \
                   p_val.squeeze()
        data_test = X_test, y_test.squeeze(), w_test.squeeze(), mu_0_test.squeeze(), \
                    mu_1_test.squeeze(), cate_test.squeeze(), p_test.squeeze()
