...
from catesel.meta_learners.base import TLearner, SLearner
from catesel.meta_learners.twostage_estimators import RALearner, DRLearner, RLearner, PWLearner
from catesel.model_selection.oracle_scorers import OracleTEScorer, OraclePOScorer
from catesel.model_selection.misc_scorers import IFTEScorer, MatchScorer
from catesel.model_selection.factual_scorers import wFactualTEScorer, FactualTEScorer
...
def run_one_experiment(input_data_train, input_data_val, input_data_test,
                       models_to_compare, criteria_to_compare, oracle_baselines=None,
                       pre_sel=True):
    ...
    for crit_name, criterion in criteria_to_compare.items():
        ...
        for model_name, model in models_to_compare.items():
            if 'oracle_p' in crit_name:
                next_val = criterion(model, X_val, y_val, w_val,
                                     p_true=p_val, t_true=cate_val)

            elif crit_name == 'po_oracle':
                next_val = criterion(model, X_val, y_val, w_val,
                                     p_true=None,
                                     t_true=[mu0_val, mu1_val])
            else:
                next_val = criterion(model, X_val, y_val, w_val,
                                     p_true=None, t_true=cate_val)
    ...
    for crit_name, best_model_name in best_models.items():
        try:
            cate_hat, po0_hat, po1_hat = models_to_compare[best_model_name].predict(X_test,
                                                                                    return_po=True)
            rmse_mu0.append(np.sqrt(mean_squared_error(po0_hat, mu0_test)))
            rmse_mu1.append(np.sqrt(mean_squared_error(po1_hat, mu1_test)))
            rmse_factual.append(
                np.sqrt(mean_squared_error(w_test * po1_hat + (1 - w_test) * po0_hat, y_test)))
        except ValueError:
    ...
    for name, model in models_to_compare.items():
        try:
            ...
            test_factual_models.append(
                np.sqrt(mean_squared_error(w_test * po1_hat + (1 - w_test) * po0_hat,
                                           y_test)))
            test_pos_models.append(mean_squared_error(po1_hat, mu1_test)/2 + mean_squared_error(
                po0_hat, mu0_test)/2)
        except ValueError:
    ...
    for crit_name, crit_model in criteria_to_compare.items():
        if 'oracle_p' in crit_name:
            real_scores.append(
                crit_model(None, X_test, y_test, w_test, p_true=p_test, t_true=cate_test,
                           t_score=cate_test))
        else:
            real_scores.append(
                crit_model(None, X_test, y_test, w_test, p_true=None, t_true=cate_test,
                           t_score=cate_test))
    ...
    if oracle_baselines is not None:
        ...
        for oracle_name, oracle_model in oracle_baselines.items():
            oracle_scores.append(np.sqrt(mean_squared_error(cate_test, oracle_model.predict(
                X_test))))

    return best_model_list + rmse_cate + real_scores + rmse_mu0 + rmse_mu1 + rmse_factual + \
           val_scores + test_pehe_models + test_factual_models + test_pos_models + oracle_scores


def run_repeat_experiment(models_to_compare, criteria_to_compare,
                          dgp_to_sample, oracle_baselines=None, n_exp=10,
                          file_name=None, crit_id=None,
                          learner_id=None, return_frame=False):
    header = ['setting_name', 'seed', 'criterion_id', 'learner_id', 'cate_var'] + \
             ...
             ['val_' + x + '_' + y for x in
              criteria_to_compare.keys() for y in models_to_compare.keys()] + \
             ...
             (['test_pehe_' + x for x in oracle_baselines.keys()] if oracle_baselines is not None
              else [])
    ...
    for i in n_exp:
        ...
        # try:
        out_i = run_one_experiment(input_data_train=data_train, input_data_val=data_val,
        ...
        if return_frame:
            new_frame = pd.DataFrame(columns=header,
                                     data=[[setting_name, i, crit_id, learner_id,
                                            np.var(data_train[5])] + out_i])
...
def get_model(model_name, hyperparams=None, n_cv=5):
    ...
    if model_name == 'lr':
        ...
        hyperparams.update(
            {key: value for key, value in hyperparam_dict.items() if key not in hyperparams.keys()})
        ...
        regressor = GridSearchCV(XGBRegressor(verbosity=0),
...
def get_learner(learner_name, learner_regressor, prop_model=None, n_folds=5):
    ...
    inner_regressor = learner_regressor if (not inner_cv) else \
        learner_regressor.estimator
    learner_regressor = clone(learner_regressor)
    inner_regressor = clone(inner_regressor)
    learner_dict = {'s': SLearner(po_estimator=learner_regressor),
                    's_ext': SLearner(po_estimator=learner_regressor,
                                      extend_covs=True),
                    't': TLearner(po_estimator=learner_regressor),
                    'dr': DRLearner(po_estimator=learner_regressor, n_folds=n_folds,
                                    te_estimator=learner_regressor,
                                    propensity_estimator=LogisticRegressionCV(
                                        Cs=[0.00001, 0.001, 0.01, 0.1, 1]) if prop_model
                                                                              is None
                                    else prop_model),
                    'dr_pocv': DRLearner(po_estimator=inner_regressor,
                                         te_estimator=inner_regressor,
                                         avg_fold_models=True,
                                         propensity_estimator=LogisticRegressionCV(
                                             Cs=[0.00001, 0.001, 0.01, 0.1, 1]) if prop_model
                                                                                   is None
                                         else prop_model, n_folds=n_folds, pre_cv_po=inner_cv,
                                         pre_cv_te=inner_cv,
                                         grid_po=param_grid
                                         ),
                    'dr_oracle_p': DRLearner(po_estimator=learner_regressor, n_folds=n_folds,
                                             te_estimator=learner_regressor,
                                             fit_propensity_estimator=False),
                    'ra': RALearner(po_estimator=learner_regressor, n_folds=1,
                                    te_estimator=learner_regressor,
                                    fit_propensity_estimator=False),
                    'r': RLearner(po_estimator=learner_regressor, n_folds=n_folds,
                                  te_estimator=learner_regressor,
                                  propensity_estimator=LogisticRegressionCV(
                                      Cs=[0.00001, 0.001, 0.01, 0.1, 1]) if prop_model
                                                                            is None
                                  else prop_model),
                    'r_pocv': RLearner(po_estimator=inner_regressor,
                                       te_estimator=inner_regressor,
                                       avg_fold_models=True,
                                       propensity_estimator=LogisticRegressionCV(
                                           Cs=[0.00001, 0.001, 0.01, 0.1, 1]) if prop_model
                    ...
                    'r_oracle_p': RLearner(po_estimator=learner_regressor, n_folds=n_folds,
                                           te_estimator=learner_regressor,
                                           fit_propensity_estimator=False),
                    'pw': PWLearner(po_estimator=learner_regressor, n_folds=1,
                                    te_estimator=learner_regressor, fit_propensity_estimator=True,
                                    propensity_estimator=LogisticRegressionCV(
                                        Cs=[0.00001, 0.001, 0.01, 0.1, 1]) if prop_model
                                                                              is None
                                    else prop_model)}
...
def get_criterion(criterion_name, criterion_regressor, prop_model=None):
    ...
    inner_regressor = criterion_regressor if (not inner_cv) else \
        criterion_regressor.estimator
    criterion_regressor = clone(
        criterion_regressor) if criterion_regressor is not None else criterion_regressor
    inner_regressor = clone(inner_regressor) if inner_regressor is not None else inner_regressor
    lr_Cs = [0.00001, 0.001, 0.01, 0.1, 1]
    crit_dict = {'factual': FactualTEScorer(mean_squared_error, 1),
                 'w_factual': wFactualTEScorer(mean_squared_error, 1,
                                               prop_model=LogisticRegressionCV(
                                                   Cs=lr_Cs) if
                                               prop_model is None else prop_model),
                 'w_factual_prefit': wFactualTEScorer(mean_squared_error, 1,
                                                      prop_model=LogisticRegressionCV(
                                                          Cs=lr_Cs) if
                                                      prop_model is None else prop_model,
                                                      plugin_prefit=True),
                 ...
                 's': PluginTEScorer(mean_squared_error, 1,
                                     po_model=SLearner(po_estimator=criterion_regressor)),
                 't': PluginTEScorer(mean_squared_error, 1,
                                     po_model=TLearner(po_estimator=criterion_regressor)),
                 's_ext': PluginTEScorer(mean_squared_error, 1,
                                         po_model=SLearner(po_estimator=criterion_regressor,
                                                           extend_covs=True)),
                 'dr_plug': PluginTEScorer(mean_squared_error, 1, po_model=DRLearner(
                     te_estimator=criterion_regressor, po_estimator=criterion_regressor,
                     propensity_estimator=LogisticRegressionCV(
                         Cs=[0.00001, 0.001, 0.01, 0.1, 1]) if prop_model
                                                               is None
                     else prop_model, n_folds=1)),
                 'r_plug': PluginTEScorer(mean_squared_error, 1, po_model=RLearner(
                     te_estimator=criterion_regressor, po_estimator=criterion_regressor,
                     propensity_estimator=LogisticRegressionCV(
                         Cs=[0.00001, 0.001, 0.01, 0.1, 1]) if prop_model
                                                               is None
                     else prop_model, n_folds=1)),
                 'ra': PseudoOutcomeTEScorer(mean_squared_error, 1,
                                             po_model=TLearner(po_estimator=criterion_regressor),
                                             pseudo_type='RA', n_folds=1),
                 'pw': PseudoOutcomeTEScorer(mean_squared_error, 1, po_model=TLearner(
                     po_estimator=criterion_regressor), prop_model=LogisticRegressionCV(
                     Cs=lr_Cs) if
                 prop_model is None else prop_model,
                                             pseudo_type='PW', n_folds=1),
                 'dr': PseudoOutcomeTEScorer(mean_squared_error, 1, po_model=TLearner(
                     po_estimator=criterion_regressor) if not inner_cv else inner_regressor,
                                             prop_model=LogisticRegressionCV(
                                                 Cs=lr_Cs) if
                                             prop_model is None else prop_model,
                                             n_folds=5, pseudo_type='DR', pre_cv_po=inner_cv,
                                             grid_po=param_grid),
                 'dr_oracle_p': PseudoOutcomeTEScorer(mean_squared_error, 1, po_model=TLearner(
                     po_estimator=criterion_regressor), fit_propensity_estimator=False,
                                                      n_folds=5, pseudo_type='DR'),
                 'r': RTEScorer(None, None, po_model=criterion_regressor if not inner_cv else
                 inner_regressor,
                                prop_model=LogisticRegressionCV(
                                    Cs=lr_Cs) if prop_model is None else prop_model,
                                pseudo_type='R', n_folds=5, pre_cv_po=inner_cv,
                                grid_po=param_grid),
                 'r_oracle_p': RTEScorer(None, None, po_model=criterion_regressor,
                                         fit_propensity_estimator=False,
                                         pseudo_type='R', n_folds=5),
                 'if': IFTEScorer(None, None, plugin_prefit=False, po_model=TLearner(
                     po_estimator=criterion_regressor) if not inner_cv else inner_regressor,
                                  prop_model=LogisticRegressionCV(Cs=lr_Cs) if
                                  prop_model is None else prop_model,
                                  n_folds=5, pre_cv_po=inner_cv, grid_po=param_grid),
                 'if_prefit': IFTEScorer(None, None, plugin_prefit=False, po_model=TLearner(
                     po_estimator=criterion_regressor), prop_model=LogisticRegressionCV(Cs=lr_Cs) if
                 prop_model is None else prop_model),
                 'dr_prefit': PseudoOutcomeTEScorer(mean_squared_error, 1,
                                                    plugin_prefit=True,
                                                    po_model=TLearner(
                                                    ...
                                                    prop_model is None else prop_model,
                                                    pseudo_type='DR'),
                 'dr_prefit_oracle_p': PseudoOutcomeTEScorer(mean_squared_error, 1,
                                                             plugin_prefit=True,
                                                             po_model=TLearner(
                                                                 po_estimator=criterion_regressor),
                                                             fit_propensity_estimator=False,
                                                             pseudo_type='DR'),
                 'ra_prefit': PseudoOutcomeTEScorer(mean_squared_error, 1,
                                                    plugin_prefit=True,
                                                    po_model=TLearner(
                                                        po_estimator=criterion_regressor),
                                                    pseudo_type='RA'),
                 'r_prefit': RTEScorer(None, None, plugin_prefit=True, po_model=criterion_regressor,
                                       prop_model=LogisticRegressionCV(
                                           Cs=lr_Cs) if prop_model is None else prop_model,
                                       pseudo_type='R'),
                 'r_prefit_oracle_p': RTEScorer(None, None, plugin_prefit=True,
