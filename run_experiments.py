...
from experiments.main_experiment import get_criterion, get_model, get_learner, run_repeat_experiment
...
DEFAULT_CRITERIA = ['factual', 'oracle',
                    's', 's_ext',
                    't', 'w_factual',
                    'dr_plug', 'r_plug', 'dr', 'r'
                    ]
...
def grid_comparison_experiment(learner_models=None,
                               crit_models=None,
                               learner_names=None,
    ...
    for reg_name in learner_models:
        for learner_name in learner_names:
            learners_to_test.update({learner_name + '-' + reg_name: get_learner(learner_name,
                                                                                get_model(
                                                                                    reg_name),
                                                                                n_folds=n_folds)})
    ...
    for crit in criterion_names:
        if crit in ['oracle', 'factual', 'po_oracle', 'w_factual', 'w_factual_prefit', 'match',
                    'pw']:
    ...
    if experiment_type == 'acic-simu':
        acic_simu_params = {'sp_nonlin': sp_nonlin,
                            'sp_lin': sp_lin,
                            'sp_1': sp_1,
                            'p_0': p_0,
                            'inter': inter,
        ...
        for param in param_loop[param_name]:
            ...
            run_repeat_experiment(learners_to_test, criteria_to_compare=criteria_to_test,
                                  dgp_to_sample=AcicLinearGenerator(**acic_simu_params),
                                  file_name=file_name, n_exp=n_exp,
                                  crit_id='mixed' if len(crit_models) > 1 else crit_models[0],
                                  learner_id='mixed' if len(learner_models) > 1 else learner_models[
                                      0], oracle_baselines=oracle_baselines)

    elif experiment_type == 'acic-orig':
        for simu_num in DEFAULT_ACIC_SIMUS:
            run_repeat_experiment(learners_to_test, criteria_to_compare=criteria_to_test,
                                  dgp_to_sample=AcicOrigGenerator(n_train=n_train, n_val=n_val,
                                                                  simu_num=simu_num),
                                  file_name=file_name,
                                  crit_id='mixed' if len(crit_models) > 1 else crit_models[0],
                                  learner_id='mixed' if len(learner_models) > 1 else learner_models[
        ...
        run_repeat_experiment(learners_to_test, criteria_to_compare=criteria_to_test,
                              dgp_to_sample=IHDPDataGenerator(setting='original',
                                                              n_train=n_train),
                              file_name=file_name, n_exp=n_exp,
                              crit_id='mixed' if len(crit_models) > 1 else crit_models[0],
                              learner_id='mixed' if len(learner_models) > 1 else learner_models[
        ...
        run_repeat_experiment(learners_to_test, criteria_to_compare=criteria_to_test,
                              dgp_to_sample=IHDPDataGenerator(setting='modified',
                                                              n_train=n_train),
                              file_name=file_name, n_exp=n_exp,
                              crit_id='mixed' if len(crit_models) > 1 else crit_models[0],
                              learner_id='mixed' if len(learner_models) > 1 else learner_models[
...
def init_arg():
    ...
    parser.add_argument('--rel_prop', default=.5, type=float)
...
if __name__ == '__main__':
    ...
    if args.experiment_type == 'acic-simu':
        if args.setup == 'A':
            grid_comparison_experiment(learner_models=args.regressors,
